...
S <- round(var(airpol.data.sub), 2)
S

#sample correlation matrix
R <- round(cor(airpol.data.sub), 2)
R
...
library(ggcorrplot)
ggcorrplot(R)#, type = "upper", outline.col = "white")
ggcorrplot(R, 
           type = "lower",
...
```{r}
std <- sapply(airpol.data.sub, sd)  #find standard deviation for variables

#divide each variable by its std to normalize the data
airpol.data.sub.std <- sweep(x = airpol.data.sub, #sweep the airpol data
